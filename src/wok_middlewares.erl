  ...
  case bucs:function_exist(Middleware, Direction, 1) of
    true ->
      case erlang:apply(Middleware, Direction,
                        [wok_message:set_local_state(Message, maps:get(Middleware, MStates, nostate))]) of
        {ok, Message1} ->
          middlewares_message(Direction, Middlewares,
                              {{ok, Message1},
                               maps:put(Middleware, wok_message:get_local_state(Message1), MStates)});
        {stop, Reason, Message1} ->
          {{stop, Middleware, Reason}, maps:put(Middleware, wok_message:get_local_state(Message1), MStates)}
      end;
    false ->
      middlewares_message(Direction, Middlewares, {{ok, Message}, MStates})
  end;
